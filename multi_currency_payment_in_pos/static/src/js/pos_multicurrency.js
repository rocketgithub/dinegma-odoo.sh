...
odoo.define('POS_multi_currency.MultiCurrency', function(require) {
    ...
    const FIXED_RATE = 0.136054; // Tasa de cambio fija

    const PaymentScreenMultiCurrency = PaymentScreen => class extends PaymentScreen {
        setup() {
            super.setup();
            current_currency = {
                rate: FIXED_RATE, // Usar la tasa fija
            };
        ...
        compute_currency(ev){
            ...
            if(currency_id){
                ...
                current_currency.rate = FIXED_RATE; // Usar la tasa fija en la conversión
                var initial_total
                if ($('.total').length > 0 ){
                ...
                $('.rate_string')[0].innerText = `1 USD = ${FIXED_RATE} ${current_currency.symbol}`; // Mostrar la tasa fija
                //Total in converted currency
